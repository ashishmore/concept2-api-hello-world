...
// Connect to the erg, and read power every 500msec.
// Note: This is per-stroke data, and doesn't always seem to reset to 0 on successive
// runs of the program!
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (ergsConnected()) {
		...
		for (int i = 0; i< 2400; i++) {
			printPower(port);
			Sleep(500);  // the data is once per stroke, so faster is not going to give much beter rez.
		}
	}
	cout << "done" << endl;
	getchar();
